...
EFT는 퀘스트 구조나 연계, 상인 평판구조까지 대대적으로 바뀌었기 때문에 SPTv4.1.x에 어울리지 않는 보상 밸런스인 EXP, 루블, 평판 보상은 적용하지 않고, 시작 조건·연계 퀘스트 같은 체인 조건은 SPT 기준을 대부분 그대로 유지합니다. 대신 킬 조건, 맵 조건, 지급 장비, 제출 아이템과 일부 보상 등을 주로 수정합니다.
...
라이트키퍼(Lightkeeper), 레프(Ref), BTR 트레이더 퀘스트는 수정 대상에서 제외합니다. 레프 퀘스트는 대부분 아레나(Arena) 연동 퀘스트라 손댈 필요가 없고, 라이트키퍼는 엔드컨텐츠 이후 퀘스트라 추후 별도로 검토할 예정입니다.
